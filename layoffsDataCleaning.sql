...
CREATE TABLE layoffs_staging -- create a copy of raw file,do not add/delete/update on the original file 
like layoffs;
...
SELECT * , -- creating row_numb column to identify duplicates
ROW_NUMBER() OVER( 
...
-- creating a copy of the copy, to add row numbers
...
SELECT `date`, -- data type incorrectly assigned
...
-- NULL and blank values identifying and deleting
...
ALTER TABLE layoffs_staging2 
DROP COLUMN row_numb; -- removing irrelavent columns
